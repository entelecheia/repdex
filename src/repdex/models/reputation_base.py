...
# define BaseChatModel type
BaseChatModel = ChatOpenAI | ChatOllama


class ReputationAspect(PydanticBaseModel):
    company: str
    aspect: str
...
class ReputationDetails(PydanticBaseModel):
    aspects: List[ReputationAspect]


class BaseReputationExtractor(BaseModel):
    _config_group_: str = "/model"
    _config_name_: str = "BaseReputationExtractor"

    llm_model: BaseModel

    _engine_: Optional[BaseChatModel] = None

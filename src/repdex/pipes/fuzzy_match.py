...
def load_defendant_names(reputation_data_file: str, target_claimant: str = "repdex"):
    """
    Load extracted defendant names from reputation data.

    Args:
        reputation_data (dict): A dictionary containing reputation information, including defendant names.

    Returns:
        list: A list of defendant names.
    """
    reputation_data = HyFI.load_jsonl(reputation_data_file)

    return [
        defentant.lower()
        for info in reputation_data
